function y = myfoo(x, a, b)
%{
myfoo  Rescale data to a specified range.

Y = myfoo(X) myfoos the elements of X to the range [0, 1]. The
minimum value of X maps to 0 and the maximum maps to 1.

Y = myfoo(X, A, B) myfoos to the range [A, B].
...
Example
  x = [1 2 3 4 5];
  y = myfoo(x)            % returns [0 0.25 0.5 0.75 1]
  y = myfoo(x, -1, 1)    % returns [-1 -0.5 0 0.5 1]

function [m, ci] = weightedmean_v6_helpsections(x, w, dim, opts)
...
%
% `opts.NanFlag` — NaN handling.
% NaN handling.  `"omitnan"` (default) excludes NaN values from the
% computation; `"includenan"` propagates them.
%
% `opts.Normalize` — Normalize weights flag.
% When `true` (default), weights are normalized to sum to 1.  Set to
% `false` when your weights are pre-normalized probabilities.
%
% `opts.Confidence` — Confidence level.
...
% on the input dimensions and operating dimension.
%
% `ci` — Confidence interval.
% Confidence interval based on the weighted standard deviation,

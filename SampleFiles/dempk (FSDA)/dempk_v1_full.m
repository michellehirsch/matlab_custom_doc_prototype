function out = dempk_v1_full(Y, k, g, opts)
...
% (`Inf`) are allowed; rows containing them are automatically excluded
% from computations.
%
% `k` — Number of components searched by the `tkmeans` algorithm.
%
% `g` — Merging rule. When `g` is an integer $\geq 1$, hierarchical
% clustering reduces components to `g` groups. When `0 < g < 1`, `g`
% is interpreted as the pairwise overlap threshold $\omega^*$ for
% merging.
%
% `opts.Alpha` — Global trimming level. When `Alpha=0` (default),
% `tkmeans` reduces to k-means.
%
% `opts.Plots` — Plot specification. Controls what is displayed after
% merging. Specify as one of:
%
% - `0` *(default)* — No plot
% - `1` — Scatter plot (histogram for 1-D, bivariate scatter for 2-D,
%   scatter matrix for higher dimensions)
...
%
% Alternatively, pass a struct with a `type` field set to one of the
% strings above, plus optional fields: `cmap` (colormap as an RGB
% matrix) or `conflev` (confidence level for ellipses, between 0 and 1).
%
% > [!NOTE]
% > Labels $\leq 0$ are automatically excluded from the overlay phase,
% > treating them as outliers.
%
% `opts.TkmeansOpt` — Optional arguments for `tkmeans`, specified as
% a structure whose field names correspond to `tkmeans` name-value
% arguments. See `tkmeans` for details.
%
% `opts.TkmeansOut` — When `true`, the `tkmeans` output structure is
% saved into the output. Default is `false`.
%
% `opts.Linkage` — Linkage method for hierarchical clustering. Default
% is `"single"`. See the MATLAB `linkage` function for valid methods.
%

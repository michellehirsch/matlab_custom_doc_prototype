...
%
% ```matlab
% s = Sensor("Pressure-A", "pressure", "Pa");
...
% See also datetime, timetable
    ...
    methods
        ...
        %
        % `s = Sensor(name, type)` creates a sensor with the given
        % name and type.
        %
        ...
        %
        % `opts.Units` — Measurement unit label. Specify as a
        % string such as `"C"`, `"Pa"`, or `"%RH"`. Default is `""`.
            arguments
        ...
        %
        % `s = reset(s)` sets `Value` to `NaN`, `Timestamp` to
        % `NaT`, and `Offset` to `0`.
            obj.Value = NaN;
            obj.Timestamp = NaT;
            obj.Offset = 0;
